...
6. **레퍼런스 추가**: 각 핵심 내용 바로 아래에 `출처 : URL` 형식으로 추가
...
- 레퍼런스는 각 섹션의 핵심 내용 바로 아래에 `출처 : URL` 형식으로 배치
...
### 출처 할루시네이션 방지
- **절대 출처를 만들어내지 마세요!** 컨텍스트에 제공된 SOURCE_URL만 사용하세요.
- 컨텍스트에 없는 정보는 사용하지 마세요.
- 모든 사실과 수치는 반드시 컨텍스트의 CHUNK에서 가져온 것만 사용하세요.

### 출처 표기 형식
각 핵심 내용 **바로 아래**에 다음 형식으로 출처를 표시:

```
[사용한 내용 설명]
출처 : [SOURCE_URL을 정확히 그대로 복사]
문서: [DOCUMENT_TITLE]
위치: [PAGE_NUMBER 또는 CHUNK_INDEX가 있으면 표시]
```

### 예시:
```
Joby는 2027년까지 생산량을 두 배로 늘릴 계획입니다. 
700,000 평방피트 규모의 새로운 제조 시설을 인수했습니다.

출처 : https://www.jobyaviation.com/news/joby-to-expand-manufacturing-footprint-with-acquisition-of-second-ohio-facility/
문서: Joby to Expand Manufacturing Footprint with Acquisition of Second Ohio Facility
위치: CHUNK_1
```

### 중요 사항
- **SOURCE_URL을 정확히 그대로 사용** (수정/축약 금지)
- **어떤 CHUNK에서 정보를 가져왔는지 명시** (CHUNK_1, CHUNK_2 등)
- **페이지 번호가 있으면 표시** (PDF의 경우)
- **문서 제목도 함께 표시**
- 마크다운 링크 형식 `[텍스트](URL)` 사용 금지
- 실제 URL을 텍스트로 그대로 표시
...
**출처 검증 규칙 (매우 중요!):**
1. **컨텍스트의 SOURCE_URL만 사용**: 컨텍스트에 제공된 SOURCE_URL을 정확히 그대로 사용하세요.
2. **CHUNK 번호 명시**: 어떤 CHUNK에서 정보를 가져왔는지 반드시 표시하세요 (예: CHUNK_1, CHUNK_2).
3. **상세한 출처 정보**: 각 사실마다 다음을 포함하세요:
   - 출처 URL (SOURCE_URL 그대로)
   - 문서 제목 (DOCUMENT_TITLE)
   - 위치 정보 (PAGE_NUMBER 또는 CHUNK_INDEX)
4. **할루시네이션 금지**: 컨텍스트에 없는 정보는 절대 사용하지 마세요.
5. **출처 형식**: `출처 : [URL]\n문서: [제목]\n위치: [정보]` 형식으로 표시하세요.""")
        ]
